...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE blog_posts SET title = ?, category = ?, content = ?, image = ? WHERE id = ?");
    $stmt->bind_param("ssssi", $title, $category, $content, $image, $id);

    if ($stmt->execute()) {
        $message = "Blog post updated successfully!";
    } else {
        $message = "Failed to update blog post: " . $conn->error;
    }
    $stmt->close();
}
?>
    ...
    <main class="ml-64 p-8 pt-24">
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg animate__animated animate__fadeIn">
            <div class="mb-4">
                ...
                <input type="text" id="title" name="title" value="<?php echo htmlspecialchars($blog['title']); ?>" required class="w-full px-3 py-2 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
            </div>
            <div class="mb-4">
                <label for="category" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Category</label>
                <select id="category" name="category" required class="w-full px-3 py-2 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400">
            ...
            <div class="mb-4">
                ...
                <textarea id="content" name="content" required class="w-full px-3 py-2 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 h-40"><?php echo htmlspecialchars($blog['content']); ?></textarea>
            </div>
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Current image: <?php echo htmlspecialchars($blog['image']); ?></p>
            </div>
